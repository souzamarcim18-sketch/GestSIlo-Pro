    ...
    try {
      ...
      if (profile?.fazenda_id) {
        ...
        if (ciclosData.data && ciclosData.data.length > 0) {
          const avgProd = ciclosData.data.reduce((acc, c) => acc + (c.produtividade || 0), 0) / ciclosData.data.length;
        ...
        if (silosData.length > 0) {
          ...
          if (silosComMS.length > 0) {
            const avgMS = silosComMS.reduce((acc, s) => acc + (s.materia_seca_percent || 0), 0) / silosComMS.length;
            ...
            <CardContent className="space-y-8">
              ...
              <div className="space-y-4">
                ...
                <p className="text-[10px] text-muted-foreground flex items-center gap-1">
                  ...
                  Média histórica da fazenda: {historicoCiclos.length > 0 ? (historicoCiclos.reduce((acc, c) => acc + (c.produtividade || 0), 0) / historicoCiclos.length).toFixed(1) : '--'} t/ha
                </p>
              </div>
              ...
              <div className="space-y-4">
                ...
                <p className="text-[10px] text-muted-foreground flex items-center gap-1">
                  <Info className="w-3 h-3" />
                  Média dos seus silos: {silos.filter(s => s.materia_seca_percent).length > 0 ? (silos.filter(s => s.materia_seca_percent).reduce((acc, s) => acc + (s.materia_seca_percent || 0), 0) / silos.filter(s => s.materia_seca_percent).length).toFixed(1) : '--'}%
                </p>
              </div>
            </CardContent>
          </Card>
