# PRD — Módulo de Estoque de Insumos (GestSilo Pro) — v2.1

**Product Requirements Document**  
Data: 2026-04-16  
Status: Em Revisão  
Versão: 2.1 (Correções e Refinamentos)
...
### 3.2 Bug #2: Queries Não Auditadas
**Arquivo:** `lib/supabase/insumos.ts`  
**Severidade:** 🔴 CRÍTICO

**Problema:**
- Funções em `insumos.ts` não validam `fazenda_id`
- Apenas RLS DB-side protege (falha em desenvolvimento sem RLS ativo)
- Violação de padrão definido em `queries-audit.ts`

**Solução:** ✅ **Deletar `lib/supabase/insumos.ts`**  
O arquivo contém apenas 4 funções (deleteInsumo, getMovimentacoesByInsumo, getTodasMovimentacoesByFazenda, createMovimentacaoInsumo) que devem ser consolidadas em `lib/supabase/queries-audit.ts` com auditoria automática de `fazenda_id`. Após migração das funções, remover o arquivo antigo e atualizar os imports em `app/dashboard/insumos/page.tsx`.
...
| **Local** | **String** | **✅ Sim** | **✅ Dropdown** | **Campo `local_armazen` (ex: Galpão 1)** |
...
- **Local**: dropdown (valores únicos de `local_armazen` da fazenda)
...
| **Tipo** | Select Dependente | Condicional | Dropdown | Depende de categoria; oculto se categoria sem subtipos |
...
| **Registrar como Despesa** | Checkbox | Opcional | Default: true | Se marcado, cria despesa automática no módulo Financeiro |
...
4. Criar despesa automática em Financeiro:
   - Adicionar checkbox no formulário: "Registrar como despesa no 
     Financeiro" (marcado por padrão ✅)
   - Se checkbox marcado: criar registro em `financeiro` com:
     * Categoria: "Insumos"
     * Descrição: "{Insumo} - Entrada de {Qtde} {Unidade}"
     * Valor: Qtde × Valor Unitário
     * Data: data da entrada
     * Referência: movimentacao_insumo.id
   - Se checkbox desmarcado: apenas registrar movimentação 
     (útil para brindes, bonificações, devoluções de cliente, etc.)
...
| **Devolução** | Fornecedor | **Text (manual)** | **Livre — não puxa automaticamente da última entrada** |
...
3. Se diferença > 0 (ganho):
   a. Criar movimentacao com:
      - tipo = 'Ajuste'
      - tipo_saida = NULL
      - quantidade = |diferença|
      - Trigger soma ao estoque_atual
4. Se diferença < 0 (perda):
   a. Criar movimentacao com:
      - tipo = 'Ajuste'
      - tipo_saida = NULL
      - quantidade = |diferença|
      - Trigger subtrai do estoque_atual
...
CREATE TABLE insumos (
  ...
  tipo_id         UUID NULL REFERENCES tipos_insumo(id),  -- FK para tipo (sub-categoria)
...
**Nota sobre `tipo_id` nullable:**  
As categorias "Sementes", "Inoculantes", "Peças e Manutenções" e "Outros" não possuem subtipos cadastrados. Para insumos dessas categorias, `tipo_id` permanece NULL. No formulário de cadastro, o campo "Tipo" deve ser ocultado ou desabilitado quando a categoria selecionada não tiver tipos associados.
...
-- ========================================
-- SEED: Tipos de Insumos (sub-categorias)
-- ========================================
...
SELECT id, unnest(ARRAY[
  ...
  'Gesso',
  'Outros'
]) FROM categorias_insumo WHERE nome = 'Fertilizantes/Corretivos';
...
SELECT id, unnest(ARRAY[
  ...
  'Adjuvantes',
  'Espalhantes'
]) FROM categorias_insumo WHERE nome = 'Defensivos';
...
SELECT id, unnest(ARRAY[
  ...
  'Gasolina',
  'Arla 32'
]) FROM categorias_insumo WHERE nome = 'Combustíveis';
...
SELECT id, unnest(ARRAY[
  ...
  'Feno',
  'Outros'
]) FROM categorias_insumo WHERE nome = 'Nutrição Animal';
...
SELECT id, unnest(ARRAY[
  'Convencional',
  'Barreira O2'
]) FROM categorias_insumo WHERE nome = 'Lonas';

-- OBS: As categorias abaixo NÃO possuem tipos cadastrados inicialmente:
-- - Sementes
-- - Inoculantes
-- - Peças e Manutenções
-- - Outros
-- Nestes casos, `tipo_id` na tabela `insumos` permanece NULL.
```
...
CREATE TABLE movimentacoes_insumo (
  ...
  -- Novos campos
  tipo_saida    VARCHAR(50),  -- 'USO_INTERNO', 'VENDA', 'DEVOLUÇÃO', etc. (somente quando tipo = 'Saída')
  ...
  CONSTRAINT check_tipo_saida CHECK(tipo_saida IS NULL OR tipo_saida IN (
    'USO_INTERNO', 'TRANSFERENCIA', 'VENDA', 'DEVOLUCAO', 'DESCARTE', 'TROCA'
  )),
...
**Nota sobre Ajuste de Inventário:**  
Movimentações do tipo `'Ajuste'` podem ter efeito de entrada (diferença positiva, ganho de estoque) ou saída (diferença negativa, perda). O campo `tipo_saida` permanece NULL para ajustes. A lógica de aplicação no `estoque_atual` é determinada pelo sinal da diferença calculada no formulário de Ajuste (seção 4.4).
...
|---|---|---|
| `insumos` | Alterada ✏️ | +categoria_id, +tipo_id (NULL), +custo_medio, +fornecedor, +local_armazen, +auditoria |
| `movimentacoes_insumo` | Alterada ✏️ | +tipo_saida (NULL para Ajuste), +destino_tipo, +destino_id, +origem, +auditoria |
...
BEGIN
  ...
  ELSIF NEW.tipo = 'Ajuste' THEN
    -- Diferença positiva/negativa registrada em `observacoes`
    -- Sinal aplicado via lógica no aplicativo (criar 2 registros se necessário)
    -- OU usar campo auxiliar de sinal. Decisão a detalhar na Spec.
    -- CMP NÃO é recalculado em ajustes.
    NULL;  -- Placeholder
  END IF;
  RETURN NEW;
END;
...
**Decisão:** ✅ **Checkbox no formulário (default: marcado)**  
Usuário tem controle granular. Casos de uso para desmarcar:  
- Insumo recebido como brinde/bonificação  
- Devolução de cliente (retorno ao estoque sem despesa)  
- Transferência entre fazendas do mesmo grupo  
- Insumo já pago em pedido anterior (apenas recebimento físico)

Fluxo:
```
Usuário clica "Cadastrar Novo Insumo" com checkbox marcado
→ Sistema cria:
...
├── supabase/
│   ├── queries-audit.ts              (Queries auditadas — única fonte)
│   └── supabase.ts                   (Types)
```
...
export const saidaFormSchema = z.object({
  ...
  tipo_saida: z.enum(['USO_INTERNO', 'VENDA', 'DEVOLUCAO', 'DESCARTE', 'TROCA']),
...
- [ ] Filtros funcionam: categoria, tipo, local, busca
...
- [ ] Campo "Tipo" oculto se categoria sem subtipos
- [ ] Validações Zod funcionam (mostra erro em tempo real)
- [ ] Salvar cria insumo + movimentação (entrada) no banco
- [ ] Checkbox "Registrar como Despesa" funciona (default: marcado)
- [ ] Despesa automática criada em Financeiro (se checkbox marcado)
- [ ] Toast de sucesso
...
- [ ] Salvar cria movimentacao tipo "Ajuste" com tipo_saida = NULL
- [ ] Estoque atualizado
...
- [ ] Colunas novas em `insumos` (categoria_id, tipo_id NULL, custo_medio, etc.)
...
- [ ] Dados iniciais (categorias/tipos) seedados com seed completo

#### 10.2.2 Queries e Mutations
- [ ] Bug #1 corrigido: fazenda_id não é vazio em create
- [ ] Bug #2 corrigido: `lib/supabase/insumos.ts` deletado, queries consolidadas
...
- [ ] Entrada de insumo cria despesa em Financeiro (se checkbox marcado)
...
|---|---|---|---|
| **1. Correções Críticas** | Week 1 | Bug fazenda_id, Deletar insumos.ts | 2-3 |
...
| **4. Frontend — Layout** | Week 3 | Alertas, listas curtas, tabela com Local | 3-4 |
...
## 13. Changelog v2 → v2.1

**Data:** 2026-04-16  
**Autor:** Revisão conjunta com stakeholder (Marcio)

### Alterações Aplicadas

1. **Seed completo de tipos** — Listagem literal de todos os tipos por 
   categoria (Correção 1)
2. **`tipo_id` nullable** — Categorias Sementes, Inoculantes, Peças e 
   Outros não têm subtipos (Correção 2)
3. **Despesa automática via checkbox** — Default marcado, editável pelo 
   usuário no form de entrada (Correção 3)
4. **Ajuste não é tipo de saída** — Removido de `tipo_saida`, mantido 
   apenas em `tipo` da tabela (Correção 4)
5. **Coluna "Local" adicionada** — Na tabela de produtos e nos filtros 
   (Correção 5)
6. **Fornecedor em Devolução** — Texto manual, não auto-preenchido 
   (Correção 6)
7. **`lib/supabase/insumos.ts` deletado** — Todas as funções 
   consolidadas em `queries-audit.ts` (Correção 7)

### Não Alterado
- Estrutura geral do documento (12 seções originais mantidas)
- Decisões técnicas principais (CMP, tabelas separadas, FK dinâmica 
  por validação TS)
- Roadmap e critérios de aceite (ajustes pontuais apenas)
