...
import type { Animal, Lote, EventoRebanho, PesoAnimal, AnimalInput, LoteInput, EventoRebanhoInput, PesoAnimalInput } from '@/lib/types/rebanho';
import {
  deleteSiloSafely,
  deleteTalhaoSafely,
  deleteCicloSafely,
...
const rebanho = {
  animais: {
    ...
    async create(payload: AnimalInput): Promise<Animal> {
      await getFazendaId();
      const { data, error } = await supabase
        .from('animais')
        .insert([payload])
    ...
    async update(id: string, payload: Partial<AnimalInput>): Promise<Animal> {
      const fazendaId = await getFazendaId();
      const { data, error } = await supabase
        .from('animais')
        .update(payload)
    ...
    async search(query: string, limit = 20): Promise<Animal[]> {
      const fazendaId = await getFazendaId();
      const { data, error } = await supabase
        .from('animais')
        .select(
  ...
  eventos: {
    async listByAnimal(animal_id: string): Promise<EventoRebanho[]> {
      const fazendaId = await getFazendaId();
      const { data, error } = await supabase
    ...
    async list(filtros?: {
      ...
      if (filtros?.tipo) query = query.eq('tipo', filtros.tipo);
      if (filtros?.data_inicio) query = query.gte('data_evento', filtros.data_inicio);
      if (filtros?.data_fim) query = query.lte('data_evento', filtros.data_fim);
    ...
    async create(payload: EventoRebanhoInput & { usuario_id: string }): Promise<EventoRebanho> {
      await getFazendaId();
      const { data, error } = await supabase
        .from('eventos_rebanho')
        .insert([payload])
  ...
  pesos: {
    async listByAnimal(animal_id: string): Promise<PesoAnimal[]> {
      const fazendaId = await getFazendaId();
      const { data, error } = await supabase
        .from('pesos_animal')
    ...
    async getLatest(animal_id: string): Promise<PesoAnimal | null> {
      const fazendaId = await getFazendaId();
      const { data, error } = await supabase
        .from('pesos_animal')
        .select('id, fazenda_id, animal_id, data_pesagem, peso_kg, observacoes, created_at')
